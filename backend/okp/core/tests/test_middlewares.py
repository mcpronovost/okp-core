...
def test_sort_admin_menu(middleware):
    ...
    with patch.object(admin.site, "get_app_list", return_value=original_app_list):
        ...
        auth_app = next(app for app in sorted_list if app["app_label"] == "auth")
        assert [model["object_name"] for model in auth_app["models"]] == ["Group", "User"]
        
        # Verify sites app models remained unchanged (no model_order defined for sites)
        sites_app = next(app for app in sorted_list if app["app_label"] == "sites")
        assert [model["object_name"] for model in sites_app["models"]] == ["Site"]
